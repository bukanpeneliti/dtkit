...
use filter_impl_lang::{parse_bool_expr, BoolExpr, FilterTranslator};
...
fn compile_filter_semantics(expr: &BoolExpr) -> Option<Expr> {
    filter_semantics::compile_bool_expr(expr)
}

mod filter_semantics {
    use polars::prelude::{col, lit, DataType, Expr};

    use super::filter_impl_lang::{
        char_at, is_identifier_continue, is_identifier_start, parse_single_call, BoolExpr, CallNode,
    };

    pub(super) fn compile_bool_expr(expr: &BoolExpr) -> Option<Expr> {
        match expr {
