...
pub(super) struct FilterTranslator;
...
impl BooleanParser {
    ...
    pub(crate) fn parse(&mut self) -> BoolExpr {
        self.parse_or(false)
    }
...
pub(crate) fn tokenize_boolean(input: &str) -> Vec<Token> {
...
pub(super) fn parse_bool_expr(input: &str) -> BoolExpr {
    let tokens = tokenize_boolean(input);
    let mut parser = BooleanParser::new(tokens);
    parser.parse()
}
...
pub(super) fn parse_single_call(input: &str) -> Option<CallNode> {
...
pub(super) fn char_at(input: &str, idx: usize) -> Option<char> {
